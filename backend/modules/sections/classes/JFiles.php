...
class JFiles {
        ...
        try {
            ...
            if (in_array($fileType[1], $default_type)) {
                ...
                if ($file->saveAs("{$filePath}.{$fileType[1]}")) {
                    ...
                    $marks = \backend\models\Watermark::find()->where('type=2')->orderBy(['default'=>SORT_DESC])->all();
                    
                    foreach($marks as $k=>$m){
                        
                        $mark = "{$path}/web/{$m['path']}/{$m['name']}";
                        $modelForm = [
                            'filename' => ($k == 0) ? "{$filePath}.{$fileType[1]}" : "{$filePath}_mark.{$fileType[1]}",
                            'mark' => $mark,
                            'target' => "{$filePath}_mark.{$fileType[1]}"
                        ];
                        
                        if($fileType[1] == 'gif')
                        {
                            $mark = "{$path}/web/{$m['path']}/{$marks[3]['name']}";
                            //$sql="convert h8Hjm.gif null: WFr1K.png  -gravity Center -layers composite -layers optimize animation.gif";
                            $sql="magick convert {$filePath}.{$fileType[1]} null: {$mark}  -gravity Center -layers composite -layers optimize {$filePath}_mark.{$fileType[1]}";
                            @exec($sql, $out, $retval);
                            
                            $sql10 = "magick convert {$filePath}_mark.{$fileType[1]} -resize 2124x1414 {$thumbnail}_2124.{$fileType[1]}";
                            $sql11 = "magick convert {$filePath}_mark.{$fileType[1]} -resize 1024x768 {$thumbnail}_1024.{$fileType[1]}";
                            $sql13 = "magick convert {$filePath}_mark.{$fileType[1]} -resize 1024x768 {$thumbnail}_mark.{$fileType[1]}";
                            $sql2 = "magick convert {$filePath}_mark.{$fileType[1]} -resize 200x200 {$thumbnail}_preview.gif";
                            
                            @exec($sql2 ." && ".$sql10 . " && " . $sql11." && ".$sql13, $out, $retval);
                ...
                if ($file->saveAs("{$filePath}.{$fileType[1]}")) {
                    ...
                    set_time_limit(1200);
                    $sql2 = "magick convert {$filePath}_mark.jpg -resize 200x200 {$thumbnail}_preview.jpg";
    ...
    public static function PdfToJpg($path, $fileName, $type = "") {
        ...
        if ($createDir) {
            set_time_limit(1200);
            $sql = "magick convert -density 800 {$path}/{$fileName} {$folderName}/preview.png";
    ...
    public static function DocToPdf($path, $fileName, $type = "") {
        ...
        $folderName = "{$path}/pdf";
        set_time_limit(1200);
        $sql = "soffice --headless --convert-to pdf:writer_pdf_Export {$path}/{$fileName} --outdir {$path}";
        exec($sql, $output, $return_var);
    ...
    public static function Ppt2Pdf($path, $fileName, $type = "") {
        $dirPath = Yii::getAlias('@storage') . "{$path}";
        $viewPath = "{$path}"; // storageUrl
        $folderName = "{$path}/pdf";
        set_time_limit(1200);
        //if($type == "xlsx" || $type == "xls"){
        $sql = "soffice --headless --convert-to pdf {$path}/{$fileName} --outdir {$path}"; //--outdir {$path}
        //}
        exec($sql, $output, $return_var);
        $fileNameArr = explode('.', $fileName);
        //self::PdfToJpg($path, "{$fileNameArr[0]}.pdf", $type);
    ...
    public static function uploadDocx($file, $filePath = "", $path = "", $fileName = "") {
        ...
        if ($file->saveAs("{$filePath}.{$file->extension}")) {//save image
            ...
            if ($file) {
                ...
                if($type == "xlsx" || $type == "xls"){
                    $detail_file = CNExcel::Excel2Array("{$filePath}.{$file->extension}");
                    $output['detai'] = \appxq\sdii\utils\SDUtility::array2String($detail_file);
                    self::Ppt2Pdf($path, "{$fileName}.{$file->extension}", $type);
                }else if($type == "docx" || $type == "doc"){
                    
                    $root_path = \Yii::getAlias('@storage').'/web/demo';
                    exec("soffice --headless --convert-to \"txt:Text (encoded):UTF8\" {$filePath}.{$file->extension} --outdir {$path}", $o,$return_var );
                    if($return_var == 0){
                        exec("soffice --headless --convert-to \"txt:Text (encoded):UTF8\" {$filePath}.{$file->extension} --outdir {$path}", $o,$return_var );
                    }
                    $detail_file = CNExcel::ReadTextFile("{$filePath}.txt");
                    $output['detai'] = $detail_file;// \appxq\sdii\utils\SDUtility::array2String($detail_file);
                    self::DocToPdf($path, "{$fileName}.{$file->extension}", $type);
                    //\appxq\sdii\utils\VarDumper::dump($output);
                    
                }else if($type == "pptx" || $type == "ppt" || $type == "odt"){
                    self::Ppt2Pdf($path, "{$fileName}.{$file->extension}", $type);
                }
...
//                }
            ...
            //\yii\helpers\Json::encode($des1) . \yii\helpers\Json::encode($des2);
