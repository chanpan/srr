            ...
            'columns' => [
               ...
               [
                   'attribute'=>'role',
                    ...
                    } ,'filter'=>["all"=>"ดูทั้งหมด","administrator"=>'แอดมิน',"manager"=>"สมาชิกศิริราช","users"=>"สมาชิกทั่วไป"],
                ],  
...
//                                        'style'=>'padding: 6px 6px 16px 6px;',
                ...
                [
                    ...
                    'value' => function ($model) {
                        $model2 = common\models\UserProfile::findOne(isset($model->userProfile->approval_by) ? $model->userProfile->approval_by : '0');
                        if($model2){
                            return "{$model2->firstname} {$model2->lastname}";
                        }else{
                            return $model->username;
                        }
                    }
...
<script>
    $('.btnImportExcelFile').on('click' , function(){
                                ...
                                let url = '<?= Url::to(['/user/import-excel'])?>';
        $('#<?= $modal?>').modal('show');
